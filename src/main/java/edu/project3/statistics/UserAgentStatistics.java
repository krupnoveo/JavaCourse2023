...
public class UserAgentStatistics implements LogStatistics {
    ...
    public Map<String, Map<String, List<String>>> getData(WrapperOfLogs logs) {
        ...
        for (Log log : logs.logs()) {
            String userAgent = log.request().userAgent();
            sourcesCount.compute(userAgent, (key, val) -> (val == null) ? 1 : val + 1);
        }
